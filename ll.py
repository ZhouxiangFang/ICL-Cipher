...
def call_llama3(tokenizer, model, fewshot_df, question, device, args):
    terminators = [
    ...
    messages = ''
    if args.prefix:
        messages += 'The following text has some words shuffled according to an unknown bijection, try to predict the output accordingly.\n'
    ...
    output = decoded[0].replace(messages, '')
    # print(output)
    # output = pl(
    #     messages,
    #     max_new_tokens=10,
...
def main():
    ...
    print('Random Sampling' if args.RS else 'Informative Sampling')
    ...
    print('Prefix' if args.prefix else 'No-Prefix')
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if 'llama' in args.model:
            output = call_llama3(tokenizer, model, fewshot_df, question, device, args)
        else:
